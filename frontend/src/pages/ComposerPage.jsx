    ...
    <div className="space-y-6 animate-fade-in max-w-4xl">
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="glass-card p-5 space-y-3"><label className="block text-sm font-medium text-theme-secondary"><MapIcon className="w-4 h-4 inline mr-2"/>{t('targetZone')}</label><div className="flex gap-2"><input type="text" value={form.target_zone} onChange={e=>updateForm('target_zone',e.target.value)} placeholder={t('zonePlaceholder')} className="input-field flex-1" id="target-zone"/><button type="button" onClick={() => setShowMapPicker(true)} title="Pick zone on map" className="btn-secondary px-3 shrink-0"><MapIcon className="w-4 h-4" /></button></div><p className="text-xs text-theme-dim">{t('zoneDesc')}</p></div>
        </div>
